...
lv_obj_t* device_page_create(lv_obj_t* tile, DevicePageParams* params)
{
    ...
    lv_device_store_snapshot_base(params->store, bases, order, &count);
    ...
    switch (d->type) {
    case LV_DEVICE_TYPE_CCT:     cct_light_page_build(tile, d); break;
    case LV_DEVICE_TYPE_CURTAIN: curtain_page_build(tile, d);   break;
...
void device_page_refresh(lv_obj_t* tile)
{
    ...
    switch (d->type) {
    case LV_DEVICE_TYPE_CCT:     cct_light_page_refresh(tile, d); break;
    case LV_DEVICE_TYPE_CURTAIN: curtain_page_refresh(tile, d);   break;
